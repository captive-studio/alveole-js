import { act } from '@/__tests__/helpers/renderNative';
import { OPTIONS, renderSelect } from '@/__tests__/helpers/selectHarness';
...
describe('Select, pilotage par la référence', () => {
  it('ouvre le panneau sur open() et le referme sur close()', async () => {
    ...
    const { queryByTestId } = await renderSelect(<Select ref={ref} options={OPTIONS} value={null} />);
  ...
  it('reste ferme quand le selecteur est desactive', async () => {
    const ref = React.createRef<SelectRef>();
    const { queryByTestId } = await renderSelect(<Select ref={ref} options={OPTIONS} value={null} disabled />);
